...
import requests
from numpy import array, digitize, histogram, mean, random
...
def analyze(args):
    ...
    try:
        ...
        tasks.append(
            RepeatingTimer(
                args.process_captcha_rates * hour_multiplier,
...
def process_captcha_rates():
    ...
    graphs = [
        Graph(
            bin_by_key="relay_exit_probability",
...
def calculate_captcha_rates(graphs, depth=1, timedelta_hours=24):
    ...
    for i in range(depth):
        ...
        captcha_rates = calculate_captcha_rates_per_consensus(
            graphs, consensus, db_relays_dict, db_urls_dict
        )
        # Save the results
...
def calculate_captcha_rates_per_consensus(
    graphs,
    consensus,
    db_relays_dict,
    db_urls_dict,
    bootstrap_sample_n=20000,
    confidence=0.95,
):
    """"""
    ...
    db_measurements = Results().get_results(
        after=consensus_time,
        before=(consensus_time + datetime.timedelta(hours=1)),
    )
    ...
    for measurement in db_measurements:
        ...
        try:
            ...
            logger.debug(
                "The requested exit relay isn't in consensus, skipping this measurement"
            )
            continue
        ...
        try:
            ...
            logger.debug(
                "This URL isn't in the database, skipping this measurement"
            )
            continue
        ...
        try:
            ...
            #   are added
            logger.debug(
                "This relay doesn't exist in the database yet, skipping this measurement"
            )
            continue

        first_seen = datetime.datetime.strptime(
            db_relay["first_seen"], "%Y-%m-%d %H:%M:%S"
        )
        ...
        measurements.append(
            Measurement(
                ...
                relay_is_ipv4_exiting_allowed=db_relay[
                    "is_ipv4_exiting_allowed"
                ],
                relay_is_ipv6_exiting_allowed=db_relay[
                    "is_ipv6_exiting_allowed"
                ],
    ...
    for graph in graphs:
        ...
        for measurement in measurements:
            if not graph.measurement_filter(
                measurement, graph.measurement_filter_args
            ):
                _measurements.remove(measurement)

        # Calculate the CAPTCHA rates
        captcha_rates = calculate_captcha_rates_per_graph(
            consensus_relays_dict, consensus_time, _measurements, graph
        )
        ...
        for n in range(bootstrap_sample_n):
            sample_n = random.choice(
                _measurements, size=sample_size, replace=True
            )
            rates = calculate_captcha_rates_per_graph(
                consensus_relays_dict, consensus_time, sample_n, graph
            )
            for rate in rates:
                try:
        ...
        for bin_key in bootstrapped_captcha_rates:
            ...
            for rate in captcha_rates:
                if rate.bin_key == bin_key:
                    ...
                    mean, lower_bound, upper_bound = mean_confidence_interval(
                        data
                    )
...
def calculate_captcha_rates_per_graph(
    consensus_relays_dict, consensus_time, measurements, graph
):
    """"""
    ...
    if bin_by_key == "relay_age":
        ...
        binned_measurements = bin_based_on_bin_boundaries(
            measurements, bin_by_key, bin_boundaries
        )
    ...
    if create_subbins_for_unique_bin_values:
        ...
        for bin_key in binned_measurements:
            # TODO - replace "exit_node" with fingerprint once measurements are indexed by fingerprint
            binned_by_exit = bin_based_on_key_values(
                binned_measurements[bin_key], "exit_node"
            )

            sub_bin_weighted_percentage, bin_sample_size = digest_captcha_rates(
                binned_by_exit, consensus_relays_dict, weighted=weighted
            )

            # Store calculated values
            captcha_rates.append(
                CAPTCHARate(
        ...
        sub_bin_weighted_percentage, bin_sample_size = digest_captcha_rates(
            binned_measurements, consensus_relays_dict, weighted=weighted
        )
...
def digest_captcha_rates(bins, consensus_relays_dict, weighted=False):
    ...
    for _bin in bins:
        ...
        captcha_percentage = (
            captcha_found_count / total_completed_measurements
        ) * 100
    ...
    if weighted:
        ...
        for _bin in bin_captcha_rates:
            bin_digested_captcha_rate += (
                bin_captcha_rates[_bin]
        ...
        for _bin in bin_captcha_rates:
            bin_digested_captcha_rate += (
                bin_captcha_rates[_bin] / total_bin_count
            )
...
def bin_based_on_key_values(list, bin_key):
    ...
    for measurement in list:
        try:
            binned[operator.attrgetter(bin_key)(measurement)].append(
                measurement
            )
