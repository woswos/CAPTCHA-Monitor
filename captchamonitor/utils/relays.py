...
class Relays:
    ...
    def get_online_relays(self):
        ...
        result = self.db.get_table_entries(
            self.db.relays_table_name, identifiers=identifiers
        )
        if result is None:
            self.logger.debug("No relay entries found")
        return result
    ...
    def get_relays_fingerprints(self):
        columns = ["fingerprint"]
        result = self.db.get_table_entries(
            self.db.relays_table_name, columns=columns
        )
        if result is None:
            self.logger.debug("No relay entries found")
        return result
    ...
    def add_relay_if_not_exists(self, data):
        self.db.insert_entry_into_table(
            self.db.relays_table_name, data, ignore_existing=True
        )
    ...
    def get_completed_jobs_for_given_relay(self, address):
        ...
        return self.db.get_table_entries(
            self.db.results_table_name, columns=columns, identifiers=identifiers
        )
