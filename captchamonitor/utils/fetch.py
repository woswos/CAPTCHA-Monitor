...
def fetch_via_method(data, timeout=30):
    ...
    method_path = os.path.join(
        os.environ["CM_BROWSER_VERSIONS_PATH"], method_to_folder[method]
    )
    ...
    if browser_version == "":
        ...
        logger.debug(
            'The latest version available for "%s" is "%s"'
    ...
    if not os.path.exists(browser_path):
        logger.warning(
            "The specified browser version %s for %s does not exist"
    ...
    logger.debug(
        'Fetching "%s" via "%s" - "v%s"' % (url, method, browser_version)
    )

    results = {}
    if method == "tor_browser":
        results = fetchers.tor_browser(
            url,
        ...
        results = fetchers.firefox_over_tor(
            url, additional_headers=additional_headers, timeout=timeout
        )

    elif method == "chromium_over_tor":
        results = fetchers.chromium_over_tor(
            url, additional_headers=additional_headers, timeout=timeout
        )
    elif method == "brave_over_tor":
        results = fetchers.brave_over_tor(
            url,
            exit_node,
            additional_headers=additional_headers,
            timeout=timeout,
        ...
        results = fetchers.requests_over_tor(
            url, additional_headers=additional_headers, timeout=timeout
        )
        ...
        results = fetchers.firefox(
            url, additional_headers=additional_headers, timeout=timeout
        )

    elif method == "chromium":
        results = fetchers.chromium(
            url, additional_headers=additional_headers, timeout=timeout
        )
    elif method == "brave":
        results = fetchers.brave(
            url, additional_headers=additional_headers, timeout=timeout
        )
    elif method == "curl":
        ...
        logger.warning(
            '"%s" is not available, please check the method name"', method
        )
        return None
