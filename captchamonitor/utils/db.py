...
class DB:
    def __init__(self):
        ...
        try:
            ...
            self.logger.error('Environment variables for the database is not set: %s', err)
            return
        ...
        try:
            ...
            self.logger.info('The database does not exist, creating it now: %s', err)
    ...
    def check_if_db_exists(self):
        ...
        sql_check_db = 'SELECT EXISTS(SELECT datname FROM pg_catalog.pg_database WHERE datname = \'%s\')' % database
    ...
    def check_if_tables_exists(self):
        ...
        for table in tables:
            ...
            sql_check_table = 'SELECT EXISTS(SELECT * FROM information_schema.tables WHERE table_name = \'%s\')' % table
    ...
    def insert_entry_into_table(self, table, data, ignore_existing=False):
        ...
        try:
            ...
            self.logger.critical('psycopg2.execute() at insert_entry_into_table() says: %s' % err)
    ...
    def get_table_entries(self, table, columns=['*'], identifiers=''):
        ...
        try:
            ...
            self.logger.critical('psycopg2.execute() at get_table_entries() says: %s' % err)
        ...
        try:
            for i in range(len(result)):
                final.append(dict(zip([c[0] for c in cursor.description], result[i])))
        except:
            final = None
    ...
    def count_table_entries(self, table):
        ...
        try:
            ...
            self.logger.critical('psycopg2.execute() at count_table_entries() says: %s' % err)
    ...
    def update_table_entry(self, table, data, identifiers=None):
        ...
        if identifiers:
            ...
            for field in identifiers:
                sql_where += ' %s=' % field
                sql_where += ' %s AND'  # this is not here for formatting but for sql
    ...
    def claim_first_uncompleted_job(self, worker_id):
        ...
        try:
            ...
            self.logger.critical(
                'psycopg2.execute() at claim_first_uncompleted_job() says: %s' % err)
