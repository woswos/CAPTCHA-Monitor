...
def run(args):
    ...
    try:
        ...
        if loop:
            logger.info(
                "Started running in the continous mode with %s worker(s)"
                % worker_count
            )
        else:
        ...
        worker_tor_base_dir = os.path.join(
            str(Path.home()), "captchamonitor", "workers"
        )
        ...
        for w_id in range(worker_count):
            ...
            p.apply_async(
                worker, args=(loop, env_var, retry_budget, timeout_value)
            )
...
def heartbeat_message():
    ...
    if last_remaining_jobs != 0:
        ...
        logger.info(
            "Heartbeat: It has been %s days, %s hours, %s minutes "
            % (int(days), int(hours), int(minutes))
        )
        logger.info("> There are %s job(s) in the queue." % remaining_jobs)
        logger.info(
            "> I processed %s job(s) since the last heartbeat"
