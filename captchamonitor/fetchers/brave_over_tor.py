...
def fetch_via_brave_over_tor(
    url, exit_node, additional_headers=None, timeout=30, **kwargs
):
    ...
    brave_browser_controlport_file = os.path.join(
        profile_path, "../../../tor/watch/controlport"
    )
    control_port = ""
    for i in range(30):
        try:
    ...
    controller = Controller.from_port(
        address="127.0.0.1", port=int(control_port)
    )
    ...
    results[
        "requests"
    ] = '{ "data": "At the moment, capturing the HTTP requests are not supported with Brave Browser over Tor" }'
