...
def fetch_via_tor_browser(
    url, additional_headers=None, security_level="medium", timeout=30, **kwargs
):
    ...
    tb_profile = os.path.join(
        tbb_path, "Browser/TorBrowser/Data/Browser/profile.default"
    )
    ...
    profile.set_preference(
        "extensions.torbutton.security_slider", security_levels[security_level]
    )
    ...
    profile.set_preference(
        "extensions.torbutton.custom.socks_host", tor_socks_host
    )
    profile.set_preference(
        "extensions.torbutton.custom.socks_port", int(tor_socks_port)
    )
    profile.set_preference(
        "extensions.torbutton.socks_port", int(tor_socks_port)
    )
